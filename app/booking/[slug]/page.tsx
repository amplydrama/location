...
export default function BookingPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    fetchVehicle();
  },[vslug]); // Depend on params.id to refetch if the slug changes
  ...
  const handleBooking = async (e:React.FormEvent) => {
    e.preventDefault()
    if (!startDate || !endDate || !customerInfo.name || 
        !customerInfo.email || !customerInfo.phone || !customerInfo.idNumber || !vehicle) {
      alert("Veuillez remplir tous les champs obligatoires (Dates et Informations personnelles).");
      return;
    }

    // You can add more robust client-side validation here (e.g., email format, phone number pattern)
    const phoneRegex = /^(6(?:7|8|5|9)\d{7}|2(?:2|3|4)\d{7})$/; // Example for MTN/Orange/Camtel and fixed lines
    if (!phoneRegex.test(customerInfo.phone)) {
        alert("Veuillez entrer un numéro de téléphone camerounais valide.");
        return;
    }
    ...
    const bookingPayload: BookingCreatePayload = {
      ...
      estimated_total_price: Math.round(totalPrice * 1.05),
      name: customerInfo.name,
      email: customerInfo.email,
      phone: customerInfo.phone,
      address: customerInfo.address,
      id_card: customerInfo.idNumber,
      with_driver: chauffeur === "1" ? true : false, // Convert to boolean
    };

    console.log("voici les donnees envoyes",bookingPayload)
    ...
    try {
      ...
      alert("Votre réservation a été enregistrée avec succès. Vous allez être redirigé pour le paiement.");
      ...
      const errorMessage = err.error || "Une erreur est survenue lors de l'enregistrement de votre réservation. Veuillez réessayer.";
