...
DO $$
BEGIN
    -- Ajouter la contrainte FK pour chat_messages si elle n'existe pas
    IF NOT EXISTS (
    ...
    -- Ajouter la contrainte FK pour chat_metrics si elle n'existe pas
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_metrics_conversation' 
        AND table_name = 'chat_metrics'
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_chat_participants_socket_id ON chat_participants(socket_id);
...
DO $$
BEGIN
    RAISE NOTICE 'Tables de chat créées avec succès !';
    RAISE NOTICE 'Vous pouvez maintenant utiliser le système de chat.';
END $$;
